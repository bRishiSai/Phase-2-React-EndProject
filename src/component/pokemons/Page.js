...
const Page = (props) => {
  ...
  return (
    <div>
      ...
      <div id='pagination'>
        <button id='previous' className='btn btn-primary' onClick={() => handlePageChange('previous')}>Previous</button>
        <span className='btn btn-primary'>Page {currentPage} of {totalPokemonPages}</span>
        <button className='btn btn-primary' id='next' onClick={() => handlePageChange('next')}>Next</button>
      </div>
    </div>
  )
}
